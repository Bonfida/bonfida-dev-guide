...
We begin by renaming the `VestingContract` object into `VestingContractHeader`, and removing the schedules from it :
...
pub struct VestingContract<'a> {
    header: &'a mut VestingContractHeader,
    schedules: &'a mut [VestingSchedule],
}
```
...
```rust
    pub fn from_buffer(
        ...
        // The first LEN bytes are occupied by the `VestingContractHeader` object
        // The rest is the schedules
        let (header, schedules) = buffer.split_at_mut(VestingContractHeader::LEN);
        Ok(Self {
            header: bytemuck::from_bytes_mut(header),
            schedules: bytemuck::cast_slice_mut(schedules),
        })
    }
```

One important thing to notice is that we're casting the _entire length_ of the buffer.
